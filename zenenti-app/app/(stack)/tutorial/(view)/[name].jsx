...
const Name = () => {
  ...
  // Zlepšenie: Optimalizácia `continueTutorial` pomocou `useCallback`
  const continueTutorial = useCallback(async () => {
    console.time("Continue Tutorial");
    if (sound) {
  ...
  const stopAndSkipToEnd = useCallback(async () => {
    console.time("Stop and Skip");
    if (sound) {
      await sound.stopAsync();
    }
    setSubtitle("");
